...
def get_file(source_dir: str, year: int) -> str:
    ...
    for f in os.listdir(source_dir):
        if f.lower() == f"c{year}_a_rev.csv":
            file: str = f
            break
    if not file:
        for f in os.listdir(source_dir):
            if f.lower() == f"c{year}_a.csv":
                file: str = f
                break

    # if no file is found, log a warning and skip this year
    if not file:
        print(f"No {type} file found for {year} in {source_dir}. Skipping {year}.")
        return None
...
def load_completions(source_dir: str, clean_dir: str, year: int, dataset_id: str) -> None:
